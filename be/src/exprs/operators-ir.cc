...
// wraps.
//
// To do this, we "type pun" the signed values to unsigned values using memcpy, which has
// defined behavior. Aliasing (unsigned * x = (unsigned *)(&y)) has undefined behavior.
// Modern compilers can "see through" the call to memcpy and generate code with no copies.
#define BINARY_OP_AS_UNSIGNED_FN(NAME, TYPE, OP)                                 \
  TYPE Operators::NAME##_##TYPE##_##TYPE(                                        \
      FunctionContext* c, const TYPE& v1, const TYPE& v2) {                      \
    if (v1.is_null || v2.is_null) return TYPE::null();                           \
    using UNSIGNED = std::make_unsigned_t<decltype(v1.val)>;                     \
    static_assert(                                                               \
        sizeof(UNSIGNED) == sizeof(v1.val), "cannot safely copy into unsigned"); \
    UNSIGNED u0, u1, u2;                                                         \
    std::memcpy(&u1, &v1.val, sizeof(UNSIGNED));                                 \
    std::memcpy(&u2, &v2.val, sizeof(UNSIGNED));                                 \
    u0 = u1 OP u2;                                                               \
    TYPE v0;                                                                     \
    std::memcpy(&v0.val, &u0, sizeof(v0.val));                                   \
    return v0;                                                                   \
  }
