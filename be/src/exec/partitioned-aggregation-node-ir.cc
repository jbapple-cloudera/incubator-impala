...
Status PartitionedAggregationNode::ProcessRow(TupleRow* __restrict__ row,
  ...
  if (AGGREGATED_ROWS) {
    ...
    UpdateTuple(dst_partition->agg_fn_ctxs.data(), it.GetTuple(), row);
    return Status::OK();
  }
...
Status PartitionedAggregationNode::AddIntermediateTuple(Partition* __restrict__ partition,
  ...
  while (true) {
    ...
    if (LIKELY(intermediate_tuple != NULL)) {
      UpdateTuple(
          partition->agg_fn_ctxs.data(), intermediate_tuple, row, AGGREGATED_ROWS);
...
bool PartitionedAggregationNode::TryAddToHashTable(
  ...
  UpdateTuple(partition->agg_fn_ctxs.data(), intermediate_tuple, in_row);
  return true;
}
